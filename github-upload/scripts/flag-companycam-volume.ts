...
// ===========================================================================
// Flags CompanyCam projects that are well-documented (25+ total photos) but
...
//   projects have 25+ photos" means (a) scanning /projects — which DOES have
...
async function main() {
  ...
  // one cheap "does it have >= threshold photos" check. Candidates that were
  // previously flagged (from an earlier run, possibly with a lower
  // threshold) but no longer qualify get explicitly UN-flagged here — the
  // merge step below is otherwise additive-only, so without this a project
  // flagged under an old, lower threshold would stay flagged forever even
  // after raising the bar.
  const previouslyFlaggedCcIds = new Set(
    existing.filter((p) => p.highVolumeUntagged && p.companycamProjectId).map((p) => p.companycamProjectId),
  );
  ...
  let unflagged = 0;
  let done = 0;
  const evaluatedProjects: Project[] = []; // both new flags AND explicit un-flags
  await mapPool(candidates, CONCURRENCY, async (raw) => {
    ...
    try {
      ...
      if (hasEnough) {
        flagged++;
        evaluatedProjects.push({ ...project, photoCount: 0, highVolumeUntagged: true });
      } else if (previouslyFlaggedCcIds.has(project.companycamProjectId)) {
        unflagged++;
        evaluatedProjects.push({ ...project, photoCount: 0, highVolumeUntagged: false });
      }
    } catch (e) {
  ...
  console.log(
    `Checked ${checked} geocodable candidate(s) (${skippedNotGeocodable} skipped — bad address/coords/service area). ` +
      `${flagged} have ${args.threshold}+ untagged photos. ${unflagged} previously-flagged project(s) no longer qualify and were un-flagged.`,
  );
  ...
  if (suppressed) console.log(`Opt-outs: suppressed ${suppressed} evaluated project(s).`);
  ...
  for (const incoming of evaluatedProjects) {
  ...
  console.log(`Merged ${flagged} newly/still-flagged and ${unflagged} un-flagged project(s) into data/projects.json.`);
  console.log("Done.");
}
